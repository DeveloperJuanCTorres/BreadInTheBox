                                ...
                                <div class="item-order__left">
                                    <div class="item-order__top">
                                        <div class="item-order__box-img">
                                            <img src="{{config('services.trading.url')}}/uploads/{{$item->attributes->image}}" alt="" class="item-order__img">
                                        </div>
                                        <div class="item-order__box-detail">
                                            <div class="item-order__detail">
                                                <p class="item-order__name"> {{ Str::limit($item->name,85)}}</p>
                                                @if($item->attributes->variante != 'DUMMY')
                                                <p class="item-sidebar__extra">
                                                        <i class="las la-bread-slice"></i>  {{ Str::limit($item->attributes->variante,25)}}
                                                </p>
                                                @endif
                                                <p class="item-order__price">$ {{number_format($item->price,2) }} </p>
                                            </div>
            ...
            <div class="cart-page__right">
                <div class="resumen-pedido">
                    ...
                    <div class="resumen-pedido__montos" >
                        <p class="resumen-pedido__subtitle ">Subtotal <span> $ {{  number_format(Cart::getSubTotal(),2)}}</span></p>
                        <p class="resumen-pedido__subtitle ">Delivery<span class="v-cloak--hidden" > $ {{  number_format( $shipping_cost ,2)}} </span></p>
                        <p class="resumen-pedido__subtitle ">Taxes<span class="v-cloak--hidden" > $ {{ number_format( $tax_cost ,2)}}</span></p>
                        <p class="resumen-pedido__subtitle resumen-pedido__subtitle--total ">Total <span>  $ {{number_format((Cart::getSubTotal() + $shipping_cost + $tax_cost),2)}}</span></p>
                    </div>
    ...
    @push('script')
       ...
       <script>
            livewire.on('register',()=>{
                ...
                Livewire.emitTo('add-suscription-cart', 'suscription_cart');
            });
        </script> 
    @endpush
